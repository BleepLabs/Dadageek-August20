...
void osc1() { // code that is run whenever the timer goes off, every 20 micros
  ...
  out1 = oscillator(0, freq1 + (lfo[1] / 2), 1, .5); //oscillator(voice select, frequency, amplitude, shape)
  out2 = oscillator(1, freq, 1 , .5); //oscillator(voice select, frequency, amplitude, shape)

  mix1 = (out1 * .5) + (out2 * .5) + (out3 *.33);
...
void loop()
{
  ...
  if (cu - previous_time[2] > time_interval[1]) {
    ...
    if (lfo[1] > 4095 ) { //4095 is the highest number we can analogWrite since we have it set to 12 bits
      lfo[1] = 4095;
      int r1 = random(100);
      if (r1 < 40) {
        lfo_latch[1] = 0;
