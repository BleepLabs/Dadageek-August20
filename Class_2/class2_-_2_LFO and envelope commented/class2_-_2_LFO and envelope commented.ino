...
void osc1() { // code that is run whenever the timer goes off, every 20 micros
  ...
  out1 = oscillator(0, freq1, mod[0], mod[3] + .25); //oscillator(voice select, frequency, amplitude, shape)
  ...
  mod[3]= (out2/4095.0)/2.0; //take the out which does from 0-4095 and make it 0-1.0, then 0-.5. This is then added to .25 to modulate the shape of the oscillator above.
