/*
  ...
  It only makes sound when the button is pressed
*/
...
void osc1() { // code that is run whenever the timer goes off, every 20 micros
  ...
  if (current_microseconds - previous_time[1] > time_interval[0]) { 
    ...
    if (lfo_latch[0] == 0) {
      lfo[0] *= .97;  //same as lfo[0]=lfo[0]*.97  lfo[] needs to be a float for this to work
    }
    if (lfo[0] < 1 ) { //4095 is the highest number we can analogWrite since we have it set to 12 bits
      lfo[0] = 0;
      lfo_latch[0] = 1;  //there is no "if" for lfo_latch==1 so it does nothing
    }
    mod[0] = lfo[0] / 4095.0;  //take the number that goes from 0-4095.0 to 0.0-1.0
    analogWrite(led1_pin, lfo[0]);
  }

  out1 = oscillator(0, freq1 + mod[2], mod[0], mod[3]); //oscillator(voice select, frequency, amplitude, shape)
  
  //we can use these oscillators as LFO too, just make them slow and change the output levels to fit your needs
...
void loop()
{
  ...
  if (button_state == 0 && prev_button_state == 1) {  //if the button goes from not pressed to pressed
    lfo_latch[0] = 0;  //make the envelope latch 0
    lfo[0] = 4095.0; //set the envelope to he high level
    Serial.println("!"); //print so let us know the button and this "if" is working
  }
  ...
  freq1 = (pot_reading[2] / 4.0)+50.0; //divide by a float to make sure we get a float out. add 50 so it never goes too low to be heard
 ...
 // here it's going pretty smoothly
  if (current_microseconds - previous_time[2] > time_interval[1]) {
    ...
    if (lfo[1] < 0) { //4095 is the highest number we can analogWrite since we have it set to 12 bits
      lfo[1] = 0; //make sure it doesn't go below 0
      lfo_latch[1]=1; //change the latch so it rises next time around
    }
    if (lfo[1] >4095) { 
      lfo[1] = 4095;
      lfo_latch[1]=0; //change the latch so it decreases next time around
    }
