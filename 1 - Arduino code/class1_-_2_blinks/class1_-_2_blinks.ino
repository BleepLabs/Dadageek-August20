//if we want a second LED we need to have seperate varibles for some things

//Varibles will not have anything inherantly in common becasue of thier names. We could name them anything.
//The varible names are only there to help us. The computer gets rid of them when we compile and upload
...
void loop()
{
  current_time = millis(); //call the function millis() to get the time in milliseconds that have ellapsed since the device started

  if (current_time - previous_time1 > interval1) { //has "interval1" amount of time past since we last executed this code?
    previous_time1 = current_time; //if the statement is true remeber the current time
    ...
    /*
        //think about why this wouldn't work
        if (led1_state == LOW) {
          led1_state = HIGH;
        }
        if (led1_state == HIGH) {
