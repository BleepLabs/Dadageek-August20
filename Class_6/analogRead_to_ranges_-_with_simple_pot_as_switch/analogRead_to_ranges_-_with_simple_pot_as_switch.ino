...
// GUItool: end automatically generated
...
float vol;
float pcell;
float step_size = 1.0 / 48.0;
float step_spacing = 40.0;
float freq;
...
void loop() {
  ...
  pot3 = (smooth(2, 31, analogRead(A2)));

  int range_size = 4095 / 3;

  if (pot3 < range_size) {
    waveform1.begin(WAVEFORM_SINE);
  }
  if (pot3 >= range_size && pot3 < range_size * 2) {
    waveform1.begin(WAVEFORM_TRIANGLE);
  }
  if (pot3 >= range_size * 2) {
  ...
  if (cm - prev[1] > 5) {
    ...
    for (float j; j < 1.0; j += step_size) { //step size controls how many little ranges there wil be
      if (pcell > j && pcell < j + step_size) { //Run thoguh all the ranges. If pcell is in that range do the thing
        note_sel = (j * step_spacing) + 10; //10 will be our lowest number
    ...
    if (note_sel != prev_note_sel) {
      new_note = 1;
      new_note_time = cm;
     // envelope1.noteOn(); //none on can only happen once so lets set a latch and remeber the time
    }

    //since we're goign to 100 milliseocds cm-new_note_time will fo from 0-100
    //multiply that by 20 to make it brighter than flip is around from 0-2000
    analogWrite(6, 2000 - ((cm - new_note_time) * 20));
    waveform1.frequency(freq);
  }

  if (new_note == 1) { //if the latch is on
    if (cm - new_note_time > 100) { //and its' been 100 millis
      new_note = 0;
      //envelope1.noteOff();
    }
  }
